...
A seção 10.2 discute a criação de `/etc/fstab`. Dadas as características do meu setup, o meu arquivo ficou assim:
...
Chequei os parâmetros de compilação do kernel duas vezes. O processo se deu sem intercorrências e durou menos de uma noite--iniciei a compilação por volta das 20h30 e, às 6h00 do outro dia, ela já havia finalizado. Como o meu plano era manter o LFS em dual boot com o host Arch, montei a partição de boot e copiei a imagem compilada para ela--no meu caso, a imagem x86_64 `vmlinuz-6.18.10-lfs-13.0-systemd`:
...
É preciso levar em consideração que o processo de atualização do GRUB conforme documentado [na Wiki do Arch](https://wiki.archlinux.org/title/GRUB)--i.e. usando `grub-mkconfig`, **mesmo com `GRUB_DISABLE_OS_PROBER=false`**--, não funciona aqui. É preciso atualizar o GRUB manualmente, conforme o próprio livro previne, no final da seção 10.4.4.
